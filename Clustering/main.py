...
data_set = {"blobs": blobs, "noisy circles": noisy_circles, "noisy moons": noisy_moons, "aniso": aniso, "no structure": no_structure, "digits": (dat, y_digits), "iris": (X_iris, y_iris)}

# data_set = {"iris": (X_iris, y_iris), "digits": (dat, y_digits), "blobs": blobs}
...
for data in data_set:
    ...
    if debug:
        ...
        benchmark(clf, "k means", sets, data, "k=%d" % k, X)
        ...
        clf = AgglomerativeClustering(linkage="average", affinity="cityblock", n_clusters=cluster,
                                      connectivity=connectivity)
        benchmark(clf, "Agglomerative Clustering", sets, data, "with number of cluster: %d" % cluster, X)
        ...
        benchmark(clf, "AffinityPropagation", sets, data,
                  "damping of %.2f" % damping, X)

        X_STD = X_STD - np.mean(X_STD, axis=0)
        X_STD /= np.std(X_STD)
        neuron_map = (cluster, 1)
        som = MiniSom(neuron_map[0], neuron_map[1], X_STD.shape[1], sigma=0.5, learning_rate=0.5, neighborhood_function='gaussian',
                      random_seed=10)
        som.pca_weights_init(X_STD)
        som.train_batch(X_STD, 1000, verbose=True)
        winner_coordinates = np.array([som.winner(x) for x in X_STD]).T
        ...
        print("jaccard score: %.2f" % metrics.jaccard_similarity_score(Y, cluster_index))
        print("normalized mutual information score: %.2f" % metrics.normalized_mutual_info_score(Y, cluster_index))
        print("silhouette score: %.2f" % metrics.silhouette_score(X_STD, cluster_index))
        # plt.figure(figsize=(neuron_map[0], neuron_map[1]))
        # frequencies = som.activation_response(X_STD)
        # plt.pcolor(frequencies.T, cmap='Blues')
        # plt.colorbar()
        # plt.show()
        if data == "digits" or data == "iris":
